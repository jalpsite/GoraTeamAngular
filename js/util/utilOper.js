		...
		$.ajax({
		    ...
		    success: function(data){
				        ...
				        if(arrayAtributos.length > 0){
							...
							$("#idButtonAddAll").prop('disabled', false);
...
$('#idButtonAddAtributo').on('click',function(){
	if($('#idEditSelectAtributo option:selected').val() != -1){
		...
		if($('#idEditSelectAtributo > option').length == 1 && 
			...
			$("#idButtonAddAll").prop('disabled', true);
		}

		$('#idEditSelectHabilidad').prop('disabled', true);
...
$('#idButtonAddAll').on('click',function(){
	if($('#idEditSelectAtributo > option').length > 0){
		$("#idEditSelectAtributo option").each(function(){
			if($(this).val() != -1){
				$('#idTableAtributo > tbody:last').append('<tr>'+
					'<td>'+$(this).val()+'</td>'+
					'<td>'+$(this).text()+'</td>'+                    
					'<td><a href="javascript:void(0);" class="remCF">Remove</a></td></tr>');
